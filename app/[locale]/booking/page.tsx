...
export default function BookingPage({
  ...
  // Handle Validation before moving to Step 2
  const handleNextStep = () => {
    let newError = { name: "", phone: "", date: "", time: "", location: "", guestCount: "" };
    let hasError = false;
    ...
    if (!formData.date.trim()) {
      newError.date = lang === "kh" ? "សូមបញ្ចូលកាលបរិច្ឆេទកម្មវិធី" : "Please enter the event date";
      hasError = true;
    }
    ...
    if (hasError) {
      toast.error(lang === "kh" ? "សូមបំពេញព័ត៌មានដែលចាំបាច់!" : "Please fill in the required fields!");
      return;
    }
    ...
    <div className={`min-h-screen bg-[#FAF7F2] ${isKh ? "font-khmer" : "font-sans"}`}>
      ...
      <main className="max-w-2xl mx-auto py-14 px-6">
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          ...
          />
          <div className={`h-0.5 w-16 rounded-full transition-all duration-500 ${step > 1 ? "bg-[#B48C00]" : "bg-slate-200"}`} />
          <StepDot
            number={2}
            active={step === 2}
            done={false}
        ...
        {step === 1 ? (
          ...
          <div className="bg-white rounded-3xl shadow-md border border-amber-100/60 overflow-hidden">
            <div className="bg-[#5C1A0B] px-8 py-6">
              ...
              <p className="text-amber-200/70 text-sm mt-1 font-medium">
                {isKh ? "សូមបំពេញព័ត៌មានខាងក្រោម" : "Please fill in the information below"}
              </p>
            </div>

            <div className="px-8 py-8 space-y-5">
              <div>
                <Field icon={<User size={18} />} label={isKh ? "ឈ្មោះអតិថិជន *" : "Full Name *"}>
                  <input
                    name="name"
                    value={formData.name}
                    placeholder={isKh ? "បញ្ចូលឈ្មោះរបស់អ្នក" : "Enter your full name"}
                ...
                {error.name && <p className="text-red-500 text-xs mt-1 ml-2 italic">{error.name}</p>}
              </div>

              <div>
                <Field icon={<Phone size={18} />} label={isKh ? "លេខទូរស័ព្ទ *" : "Phone Number *"}>
                  <input
                    name="phone"
                    value={formData.phone}
                    placeholder={isKh ? "ឧ. +855 12 345 678" : "e.g. +855 12 345 678"}
                    className="w-full bg-transparent outline-none text-slate-800 font-semibold placeholder:text-slate-300 placeholder:font-normal"
                    onChange={(e) => {
                ...
                {error.phone && <p className="text-red-500 text-xs mt-1 ml-2 italic">{error.phone}</p>}
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Field icon={<Calendar size={18} />} label={isKh ? "កាលបរិច្ឆេទ *" : "Event Date *"}>
                    <input
                      name="date"
                      value={formData.date}
                      type="date"
                  ...
                  {error.date && <p className="text-red-500 text-xs mt-1 ml-2 italic">{error.date}</p>}
                </div>
                
                <Field icon={<Clock size={18} />} label={isKh ? "ម៉ោង" : "Time"}>
                  <input
                    name="time"
                    value={formData.time}
                    type="time"
              ...
              <Field icon={<MapPin size={18} />} label={isKh ? "ទីតាំងកម្មវិធី" : "Event Location"}>
                <input
                  name="location"
                  value={formData.location}
                  onChange={(e) => setFormData({ ...formData, location: e.target.value })}
                  placeholder={isKh ? "ឧ. ភ្នំពេញ, ខណ្ឌចំការមន" : "e.g. Phnom Penh, Chamkarmon"}
                  className="w-full bg-transparent outline-none text-slate-800 font-semibold placeholder:text-slate-300 placeholder:font-normal"
                />
              </Field>
              ...
              {cartItems.length > 0 && (
                <div className="mt-2 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-xs text-slate-400 font-bold">{isKh ? "តម្លៃសរុប" : "Total"}</p>
                    <p className="text-xl font-black text-emerald-600">${totalPrice.toFixed(2)}</p>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-3xl shadow-md border border-amber-100/60 overflow-hidden">
            <div className="bg-[#5C1A0B] px-8 py-6">
              ...
              <p className="text-amber-200/70 text-sm mt-1 font-medium">
                {isKh ? "សូមពិនិត្យព័ត៌មានមុននឹងបញ្ជាក់" : "Please review your details before confirming"}
              </p>
            </div>

            <div className="px-8 py-8 space-y-5">
              <div className="rounded-2xl border border-slate-100 overflow-hidden divide-y divide-slate-100">
                <SummaryRow icon={<User size={16} />} label={isKh ? "ឈ្មោះ" : "Full Name"} value={formData.name || "—"} />
                <SummaryRow icon={<Phone size={16} />} label={isKh ? "លេខទូរស័ព្ទ" : "Phone"} value={formData.phone || "—"} />
                <SummaryRow icon={<Calendar size={16} />} label={isKh ? "កាលបរិច្ឆេទ" : "Date"} value={formData.date || "—"} />
                {formData.time && <SummaryRow icon={<Clock size={16} />} label={isKh ? "ម៉ោង" : "Time"} value={formData.time} />}
                {formData.location && <SummaryRow icon={<MapPin size={16} />} label={isKh ? "ទីតាំង" : "Location"} value={formData.location} />}
              </div>
              ...
              <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-2xl px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <p className="font-black text-slate-700 text-sm uppercase tracking-wider">{isKh ? "តម្លៃសរុប" : "Total Price"}</p>
                </div>
                <p className="text-2xl font-black text-emerald-600">${totalPrice.toFixed(2)}</p>
              </div>
...
function StepDot({ number, active, done, label }: { number: number; active: boolean; done: boolean; label: string }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className={`w-9 h-9 rounded-full font-black text-sm flex items-center justify-center border-2 transition-all duration-300 ${done ? "bg-[#B48C00] border-[#B48C00] text-white" : active ? "bg-[#5C1A0B] border-[#5C1A0B] text-white shadow-lg shadow-[#5C1A0B]/20" : "bg-white border-slate-200 text-slate-300"}`}>
        {done ? <CheckCircle size={16} /> : number}
      </div>
      <span className={`text-[10px] font-black uppercase tracking-widest ${active ? "text-[#5C1A0B]" : "text-slate-300"}`}>{label}</span>
    </div>
  );
}
