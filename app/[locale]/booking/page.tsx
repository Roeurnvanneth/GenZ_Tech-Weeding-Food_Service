...
import { User, Phone, Calendar, Clock, MapPin, ChevronRight, ChevronLeft, CheckCircle, ShoppingBag, DollarSign } from 'lucide-react';

export default function BookingPage({ params }: { params: Promise<{ locale: Language }> }) {
    ...
    const handleConfirmBooking = async () => {
        if (!formData.name || !formData.phone || !formData.date) {
            toast.error(lang === 'kh' ? "សូមបំពេញព័ត៌មានឱ្យគ្រប់គ្រាន់!" : "Please fill in all required fields!");
            return;
        }
        ...
        <div className={`min-h-screen bg-[#FAF7F2] ${isKh ? 'font-khmer' : 'font-sans'}`}>
            ...
            <main className="max-w-2xl mx-auto py-14 px-6">
                ...
                <div className="flex items-center justify-center gap-3 mb-10">
                    <StepDot number={1} active={step === 1} done={step > 1} label={isKh ? "ព័ត៌មាន" : "Details"} />
                    <div className={`h-0.5 w-16 rounded-full transition-all duration-500 ${step > 1 ? 'bg-[#B48C00]' : 'bg-slate-200'}`} />
                    <StepDot number={2} active={step === 2} done={false} label={isKh ? "បញ្ជាក់" : "Confirm"} />
                </div>
                                ...
                                <div className="mt-2 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-slate-600 font-bold text-sm">{cartItems.length} {isKh ? "មុខ" : "items"}</p>
                                        </div>
                                    </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-md border border-amber-100/60 overflow-hidden">
                        ...
                        <div className="px-8 py-8 space-y-5">
                            ...
                            <div className="rounded-2xl border border-slate-100 overflow-hidden divide-y divide-slate-100">
                                ...
                                {formData.time && (
                                    <SummaryRow icon={<Clock size={16} />} label={isKh ? "ម៉ោង" : "Time"} value={formData.time} />
                                )}
                                {formData.location && (
                                    <SummaryRow icon={<MapPin size={16} />} label={isKh ? "ទីតាំង" : "Location"} value={formData.location} />
                                )}
                            </div>
...
function StepDot({ number, active, done, label }: { number: number, active: boolean, done: boolean, label: string }) {
    return (
        <div className="flex flex-col items-center gap-1.5">
            <div className={`w-9 h-9 rounded-full font-black text-sm flex items-center justify-center border-2 transition-all duration-300 ${
                done ? 'bg-[#B48C00] border-[#B48C00] text-white' :
                active ? 'bg-[#5C1A0B] border-[#5C1A0B] text-white shadow-lg shadow-[#5C1A0B]/20' :
                'bg-white border-slate-200 text-slate-300'
            }`}>
                {done ? <CheckCircle size={16} /> : number}
            </div>
            <span className={`text-[10px] font-black uppercase tracking-widest ${active ? 'text-[#5C1A0B]' : 'text-slate-300'}`}>
                {label}
            </span>
        </div>
    );
