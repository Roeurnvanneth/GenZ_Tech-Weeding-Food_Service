...
import { useSearchParams, useRouter, useParams } from "next/navigation"; // បន្ថែម useParams
import { useState, useEffect } from "react";

export default function VerifyOTP() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const { locale } = useParams(); // ចាប់យក locale ពី URL (ឧទាហរណ៍: kh)
  ...
  const handleVerify = async (inputOtp: string) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        // ១. រក្សាទុក Token និង Role ក្នុង LocalStorage
        localStorage.setItem("token", data.token);
        localStorage.setItem("role", "USER");

        // ២. កែប្រែត្រង់នេះ៖ រុញទៅ Homepage តាម Locale (ឧទាហរណ៍: /kh/)
        router.push(`/${locale}/`); 
      } else {
